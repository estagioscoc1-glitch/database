...
export const CalendarioEscolarPrintView: React.FC<Props> = ({ registro, onClose }) => {
  ...
  const Documento = (
    ...
    >
        ...
        <div style={{ flex: '1', display: 'flex', flexDirection: 'column', gap: '0.4cm' }}>
          {registro.dados.anotacoes.length > 0 && (
            <div style={{ border: '1px solid #cbd5e1', borderRadius: '10px', padding: '0.45cm' }}>
              ...
              <div style={{ margin: 0, fontSize: '9.5pt', lineHeight: 1.5 }}>
          ...
          <div style={{ border: '1px solid #0f172a', borderRadius: '10px', overflow: 'hidden', fontSize: '10.5pt' }}>
            <div style={{ background: '#0f172a', color: '#fff', padding: '0.25cm 0.4cm', fontWeight: 'bold', textTransform: 'uppercase', fontSize: '9.5pt' }}>
              Orientações e dias letivos
            </div>
            {registro.dados.meses.map(m => (
              <div
                key={m.mes}
                style={{ display: 'flex', justifyContent: 'space-between', padding: '0.18cm 0.4cm', borderBottom: '1px solid #e2e8f0' }}
              >
                <span style={{ fontWeight: 'bold' }}>{NOMES_MESES[m.mes]}</span>
                <span>{m.diasLetivos} Dias</span>
              </div>
            ))}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.25cm 0.4cm', background: '#f1f5f9', fontWeight: 'bold' }}>
              <span>Total</span>
          ...
          {registro.dados.fraseRodape && (
            <p style={{ fontSize: '9.5pt', fontStyle: 'italic', color: '#475569', textAlign: 'center', margin: 0 }}>
              {registro.dados.fraseRodape}
            </p>
          )}
        </div>
      ...
      <div style={{ marginTop: '0.6cm', paddingTop: '0.3cm', borderTop: '1px solid #cbd5e1', textAlign: 'center', fontSize: '8pt', color: '#444', lineHeight: 1.4 }}>
