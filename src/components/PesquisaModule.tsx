                      ...
                      const html = `
                        <div style="font-family: Arial, sans-serif; padding: 25px; color: #0f172a;">
                          ...
                          <table style="width: 100%; font-size: 12px; margin-bottom: 20px; line-height: 1.6;">
                            <tr><td><strong>Nome Completo:</strong> ${escapeHtml(selectedStudent.name)}</td><td><strong>Matrícula:</strong> ${escapeHtml(selectedStudent.enrollment || '-')}</td></tr>
                            <tr><td><strong>CPF:</strong> ${escapeHtml(selectedStudent.cpf || selectedStudentDetails?.cpf || '-')}</td><td><strong>RG:</strong> ${escapeHtml(selectedStudentDetails?.rg || '-')} (${escapeHtml(selectedStudentDetails?.rgIssuer || 'SSP')}/${escapeHtml(selectedStudentDetails?.rgUf || 'PB')})</td></tr>
                            <tr><td><strong>Mãe:</strong> ${escapeHtml(selectedStudentDetails?.motherName || '-')}</td><td><strong>Pai:</strong> ${escapeHtml(selectedStudentDetails?.fatherName || '-')}</td></tr>
                            <tr><td><strong>Nascimento:</strong> ${escapeHtml(selectedStudentDetails?.birthDate || '-')}</td><td><strong>Estado Civil:</strong> ${escapeHtml(selectedStudentDetails?.maritalStatus || 'Solteiro(a)')}</td></tr>
                            <tr><td><strong>WhatsApp:</strong> ${escapeHtml(selectedStudentDetails?.whatsapp || selectedStudent.phone || '-')}</td><td><strong>E-mail:</strong> ${escapeHtml(selectedStudent.email || '-')}</td></tr>
                            <tr><td><strong>Endereço:</strong> ${escapeHtml(selectedStudentDetails?.address || 'Rua Principal, 100')} - ${escapeHtml(selectedStudentDetails?.neighborhood || 'Centro')}, ${escapeHtml(selectedStudentDetails?.city || 'João Pessoa')} - ${escapeHtml(selectedStudentDetails?.state || 'PB')}</td><td><strong>CEP:</strong> ${escapeHtml(selectedStudentDetails?.cep || '58000-000')}</td></tr>
                          </table>
                    ...
                    onClick={() => {
                      ...
                      const rows = stInstallments.map(inst => `
                        <tr>
                          ...
                          <td style="padding: 6px; border: 1px solid #ddd;">${escapeHtml(inst.dueDate)}</td>
                          <td style="padding: 6px; border: 1px solid #ddd;">${escapeHtml(inst.paidAt || '-')}</td>
                          <td style="padding: 6px; border: 1px solid #ddd;">${escapeHtml(inst.paidMethod || '-')}</td>
                          <td style="padding: 6px; border: 1px solid #ddd;">${escapeHtml(inst.receiptNumber || '-')}</td>
                          <td style="padding: 6px; border: 1px solid #ddd; font-weight: bold;">${escapeHtml(inst.status)}</td>
                        </tr>
                      `).join('');
                      ...
                      const html = `
                        <div style="font-family: Arial, sans-serif; padding: 20px;">
                          ...
                          <p style="text-align: center; font-size: 12px; color: #475569;">Aluno: <strong>${escapeHtml(selectedStudent.name)}</strong> | CPF: ${escapeHtml(selectedStudent.cpf || '-')} | Matrícula: ${escapeHtml(selectedStudent.enrollment || '-')}</p>
