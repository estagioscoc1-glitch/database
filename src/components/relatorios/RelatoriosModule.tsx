...
export const RelatoriosModule: React.FC<RelatoriosModuleProps> = ({ initialSubTab = 'contratos' }) => {
  ...
  const replaceDocumentTags = (templateHtml: string, student: User, details: DetailedStudent | null) => {
    ...
    return templateHtml
      ...
      .replace(/\{CPF\}/g, stCpf)
      .replace(/\{RG\}/g, stRg)
      .replace(/\{MATRICULA\}/g, stEnrollment)
      .replace(/\{CURSO\}/g, stCourse)
      .replace(/\{TURMA\}/g, stClass)
      .replace(/\{TURNO\}/g, 'Manhã / Noite')
      .replace(/\{SEMESTRE\}/g, selectedSemester)
      .replace(/\{ANO_LETIVO\}/g, selectedYear)
      .replace(/\{ENDERECO\}/g, stAddress)
                  ...
                  onClick={() => {
                    const rowsHtml = studentsList.slice(0, 10).map((st, i) => `
                      <tr>
                        ...
                        <td style="padding: 6px; border: 1px solid #ddd;">${st.name}</td>
                        <td style="padding: 6px; border: 1px solid #ddd;">${st.enrollment || '2026.1.ENF.089'}</td>
                  ...
                  <button
                    onClick={() => {
                      setPrintModal({
                        ...
                        contentHtml: `<pre style="font-family: monospace; font-size: 11px;">${txtRows}</pre>`,
                        txtData: txtRows,
