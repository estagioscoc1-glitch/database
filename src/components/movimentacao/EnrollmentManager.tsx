...
export const EnrollmentManager: React.FC<EnrollmentManagerProps> = ({ currentUser }) => {
  ...
  const handleGenerateContract = (enr: StudentEnrollment) => {
    ...
    const content = contractTpl.contentHtml
      .replace(/{NOME_ALUNO}/g, enr.studentName)
      .replace(/{CPF}/g, enr.studentCpf)
      .replace(/{MATRICULA}/g, enr.enrollmentNumber)
      .replace(/{CURSO}/g, enr.courseName)
      .replace(/{TURMA}/g, enr.className)
      .replace(/{TURNO}/g, enr.shift)
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
          <tr><td style="padding: 6px; font-weight: bold; width: 30%;">Nome do Aluno:</td><td style="padding: 6px;">${enr.studentName}</td></tr>
          <tr><td style="padding: 6px; font-weight: bold;">CPF:</td><td style="padding: 6px;">${enr.studentCpf}</td></tr>
          <tr><td style="padding: 6px; font-weight: bold;">Número de Matrícula:</td><td style="padding: 6px; font-family: monospace; font-weight: bold;">${enr.enrollmentNumber}</td></tr>
          <tr><td style="padding: 6px; font-weight: bold;">Curso Escolhido:</td><td style="padding: 6px;">${enr.courseName}</td></tr>
          <tr><td style="padding: 6px; font-weight: bold;">Turma e Turno:</td><td style="padding: 6px;">${enr.className} (${enr.shift})</td></tr>
          <tr><td style="padding: 6px; font-weight: bold;">Sala de Aula:</td><td style="padding: 6px;">${enr.roomName || 'Sala Central'}</td></tr>
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 25px; line-height: 1.8;">
        ...
        <p style="text-align: justify; margin-top: 30px;">
          A Direção Geral do Colégio e Instituto Técnico Oswaldo Cruz declara, para os devidos fins de direito e comprovação junto aos órgãos de transporte público e regulação (CETRAN/DER), que o(a) estudante <strong>${enr.studentName}</strong>, inscrito(a) no CPF sob nº <strong>${enr.studentCpf}</strong> e Matrícula nº <strong>${enr.enrollmentNumber}</strong>, encontra-se regularmente MATRICULADO(A) e FREQUENTANDO as aulas do curso de <strong>${enr.courseName}</strong>, Turma <strong>${enr.className}</strong>, no Turno <strong>${enr.shift}</strong>.
        </p>
