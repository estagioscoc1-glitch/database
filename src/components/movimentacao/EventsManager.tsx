...
export const EventsManager: React.FC<EventsManagerProps> = ({ currentUser }) => {
  ...
  const handleGenerateCertificate = async (evt: EventMinicourse, part: EventParticipant) => {
    ...
    `)
      .replace(/{NOME_ALUNO}/g, part.studentName)
      .replace(/{CPF}/g, part.enrollmentNumber)
      .replace(/{CURSO}/g, evt.title)
      .replace(/{CARGA_HORARIA}/g, evt.workloadHours.toString())
      .replace(/{DATA}/g, new Date(evt.date).toLocaleDateString('pt-BR'))
      .replace(/{PROFESSOR}/g, evt.instructor);
