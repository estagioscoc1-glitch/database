...
const SECOES: Secao[] = [
  ...
  },
  {
    ...
    itens: [
      ...
      { titulo: 'Contratos — Gerar', texto: 'Aba dentro de Requerimentos. Busca o aluno e monta o Contrato de Prestação de Serviços Educacionais preenchido sozinho: nome, estado civil, CPF, RG e endereço vêm da ficha do aluno; o curso e o módulo vêm da turma em que ele está matriculado. Ou seja, um aluno do 2º módulo recebe o contrato já escrito \'2º módulo\'. Se o aluno estiver sem turma, os campos aparecem em vermelho pedindo preenchimento à mão — o sistema não inventa módulo, porque contrato assinado com módulo errado vira problema jurídico. Os valores (total, entrada, parcelas, descontos por turno) vêm sugeridos do contrato oficial e podem ser mudados antes de gerar.' },
      { titulo: 'Contratos — Editar Modelo', texto: 'O texto do contrato pode ser editado pela própria secretaria, cláusula por cláusula e parágrafo por parágrafo, sem precisar chamar programador. São dois modelos separados: Presencial e EAD. Ao salvar, a versão editada passa a valer em todo contrato novo; os que já foram impressos não mudam. O botão \'Voltar ao original\' descarta as edições e devolve o texto de fábrica. Dentro do editor tem a lista dos campos que se preenchem sozinhos, como {{NOME}}, {{MODULO}} e {{CURSO}}.' },
      { titulo: 'Histórico Escolar — Como funciona', texto: 'Aba dentro de Requerimentos. Gera o histórico do aluno, completo ou parcial. O parcial é o que a escola chama de Modelo de Transferência: as disciplinas não cursadas saem como À Cursar e o estágio como à cursar. São quatro modelos, um por curso: Enfermagem, Enfermagem EAD, Radiologia e Segurança do Trabalho. Cada um tem resolução, carga horária, horas de estágio e lista de competências próprias. As grades foram extraídas das planilhas oficiais da escola, não digitadas à mão. Sai em duas folhas: as notas na primeira e as competências na segunda.' },
      { titulo: 'Histórico Escolar — Trava por curso', texto: 'Igual à da Declaração de Auxiliar. Se o curso do aluno não bater com nenhum dos quatro modelos, a tela não deixa gerar e explica o motivo, porque emitir com o modelo de outro curso geraria um documento com resolução, carga horária e competências erradas. Um aluno de Técnico em Enfermagem EAD cai no modelo EAD e não no presencial, porque o casamento é feito pelo termo mais específico do nome do curso.' },
      { titulo: 'Documentos — A modalidade EAD nunca sai impressa', texto: 'Histórico, declaração, diploma e ficha de estágio trazem apenas Técnico em Enfermagem, sem EAD e sem Semipresencial. O que o aluno conclui é o Técnico em Enfermagem, com o mesmo registro, tenha cursado presencial ou a distância. O EAD aparece só na tela, para o atendente conferir qual modelo foi escolhido, e no contrato, onde a modalidade contratada é informação relevante. O curso EAD tem grade própria, com Módulo Básico, Intermediário e Avançado — a mesma cadastrada em Grades Curriculares, e não uma cópia da grade presencial.' },
      { titulo: 'Histórico Escolar — Resolução e conceito', texto: 'O cabeçalho vem com a resolução padrão do curso, mas você pode escolher outra na lista ou digitar uma que não esteja lá. Isso é necessário porque o histórico de aluno antigo precisa trazer a resolução vigente na época em que ele cursou, e não a de hoje. A coluna CONCEITO imprime letra: A de 86 a 100, B de 76 a 85, C de 60 a 75 e D de 00 a 59. A nota do portal, de 0 a 10, é convertida automaticamente. Disciplina sem nota lançada sai com traço e não com D, porque não lançada é diferente de reprovado. Antes de gerar, a tela avisa quantas disciplinas estão sem nota.' },
      { titulo: 'Ficha de Estágio — Como funciona', texto: 'Aba dentro de Requerimentos. Monta o Resumo das Avaliações dos Estágios Curriculares, que vai assinado pela secretaria, pela coordenação de estágio e pela direção. Busca o aluno e traz os estágios que ele já tem lançados no módulo de Estágios: componente, carga horária, unidade médica, nota e supervisor. Nenhuma nota é digitada aqui — a nota continua sendo lançada onde sempre foi. Imprima em paisagem, porque a tabela é larga.' },
      { titulo: 'Ficha de Estágio — As notas', texto: 'As cinco colunas da ficha (conhecimento técnico, habilidade técnica, atitudes pessoais, valores éticos e média final) repetem a MESMA nota lançada naquele estágio. Não são cinco notas diferentes — é como a escola preenche na prática. A única conta que o sistema faz é a média geral no pé da ficha: média simples das notas dos componentes, sem peso por carga horária. Componente sem nota sai com traço e fica de fora do cálculo, e a tela avisa quantos estão assim. Atenção: a planilha antiga tinha um erro de fórmula na média geral, dividindo por 101 em vez de pelo número de componentes (mostrava 8,2178 onde o certo era 8,30). Aqui a conta é feita certa, então as médias podem sair um pouco diferentes das fichas antigas.' },
      { titulo: 'Ficha de Estágio — Registro e vistos', texto: 'O único campo editável na tela é o registro do supervisor no conselho (COREN, CRTR), que aparece impresso ao lado do nome dele e não existia no cadastro de estágio. Salva sozinho quando você sai do campo. No botão Cabeçalho e vistos dá para ajustar, por curso, a resolução impressa no topo e os nomes e cargos dos três vistos — Enfermagem usa a Resolução CEE/GO nº 018/2022 e Radiologia a CEE 041/2022, por isso a configuração é separada por curso.' },
      { titulo: 'Contratos — Aditivo de Dependência', texto: 'Quando o aluno escolhido está matriculado em dependência, aparece um quadro amarelo na tela de gerar contrato listando as disciplinas e oferecendo o Termo Aditivo de Matrícula em Dependência. É um documento separado, assinado junto com o contrato. O valor vem sugerido como R$ 80,00 por mês em 6 parcelas, e os dois campos são editáveis antes de gerar. O sistema descobre a dependência sozinho por dois caminhos: o cadastro formal com situação ATIVO e as turmas marcadas como dependência — consulta os dois porque, dependendo de como o cadastro foi feito, a informação pode estar só num deles.' },
      { titulo: 'Contratos — Visto em todas as folhas', texto: 'Como a escola exige que o aluno viste cada página, o timbre do colégio se repete no topo de todas as folhas e a linha \'Visto do(a) contratante\' se repete no pé de todas as folhas. Por isso é obrigatório desmarcar \'Cabeçalhos e rodapés\' em Mais definições na caixa de impressão: se ficar marcado, a URL do portal sai por cima do timbre e a data por cima da linha de visto.' },
      { titulo: 'Declarações — Gerar', texto: 'Aba dentro de Requerimentos. São cinco modelos: Conclusão, Auxiliar de Enfermagem, Escolaridade, SETRANSP e Vacina. Busca o aluno e preenche sozinho o nome, a matrícula, a filiação, a data de nascimento e a naturalidade a partir da ficha, e o curso, o módulo e o turno a partir da turma. O que não existe no cadastro, como a data em que o aluno concluiu o curso, aparece como caixa para preencher à mão. Antes de gerar, um aviso amarelo lista o que está faltando, para você não descobrir só depois de imprimir. Sai com o timbre e a assinatura do secretário.' },
      { titulo: 'Declarações — Trava por curso', texto: 'A Declaração de Auxiliar de Enfermagem só pode ser emitida para aluno de Enfermagem. Ela atesta a conclusão dos módulos I e II do Técnico em Enfermagem, então para aluno de Segurança do Trabalho, Radiologia ou qualquer outro curso o documento seria falso. Nesses casos o botão fica cinza com um cadeado, e mesmo que algum caminho novo deixe passar pelo botão, a geração é recusada com um aviso. Se você trocar de aluno depois de já ter escolhido a declaração, o sistema volta sozinho para uma que sirva ao curso do novo aluno. O mesmo valerá para o Certificado de Auxiliar quando for construído.' },
      { titulo: 'Declarações — Editar Modelos', texto: 'O texto de cada uma das cinco declarações pode ser editado pela secretaria, parágrafo por parágrafo, incluindo o título e se mostra ou não a assinatura e o rodapé com o endereço. Ao salvar, a versão editada passa a valer nas declarações novas. O botão Voltar ao original descarta as edições. Importante: as declarações de Escolaridade, SETRANSP e Vacina que o aluno emite sozinho no painel dele usam ESTE MESMO modelo. Editar o texto aqui muda também o que sai pelo painel do aluno — não existem mais dois textos diferentes do mesmo documento.' },
